								   ...
								   const char *dbspacedirname, int op)
{
	...
	if ((op & UNLOGGED_RELATION_INIT) != 0)
	{
		...
		while ((de = ReadDir(dbspace_dir, dbspacedirname)) != NULL)
		{
			 ...
			 * was produced under (fork=INIT_FORKNUM, blocknum) AAD and a
			 * raw byte copy into the MAIN fork would not decrypt later
			 * (the read path supplies fork=MAIN_FORKNUM as AAD).  Decrypt
			 * INIT-side and re-encrypt MAIN-side instead so the AAD
			 * matches at read time.
			 */
 ...
 *
 * Each INIT-fork page was encrypted with AAD identifying it as INIT_FORKNUM;
 * a raw byte copy into MAIN would not decrypt afterwards because the
 * encryption layer supplies fork=MAIN_FORKNUM as AAD on read.  Decrypt each
 * block of the INIT segment file and re-encrypt under MAIN's AAD using the
 * relation's existing DEK (read from the KEY fork), producing a MAIN segment
 * file that the running cluster can read normally.
 *
