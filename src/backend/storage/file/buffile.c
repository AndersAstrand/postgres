...
struct BufFile
{
	...
	/*
	 * Reusable encryption-side buffers; lazily allocated on first encrypted
	 * I/O so non-encrypted BufFiles pay no allocator cost.  Sized to the
	 * maximum per-block ciphertext / plaintext footprint for the loaded
	 * module.
	 */
	char	   *enc_ciphertext_buf;
	char	   *enc_plaintext_buf;
...
makeBufFileCommon(int nfiles)
{
	...
	file->pos = 0;
	file->nbytes = 0;
	file->enc_ciphertext_buf = NULL;
	file->enc_plaintext_buf = NULL;
 ...
 *
 * The ciphertext buffer needs room for BLCKSZ plaintext plus the loaded
 * module's per-record overhead.  Enforce that the result still fits in a
 * physical BufFile block; modules with very large overhead can't be used
 * for BufFile.
 */
static inline void
BufFileEnsureEncBuffers(BufFile *file)
{
	if (file->enc_ciphertext_buf == NULL)
	{
		MemoryContext oldcontext;
		Size		max_ciphertext = BLCKSZ + FileEncryptionOverheadSize();

		if (max_ciphertext > BUFFILE_MAX_CIPHERTEXT)
			ereport(ERROR,
					(errcode(ERRCODE_INTERNAL_ERROR),
					 errmsg("file encryption module overhead %zu exceeds BufFile per-block limit %zu",
							FileEncryptionOverheadSize(),
							(Size) (BUFFILE_MAX_CIPHERTEXT - BLCKSZ))));

		oldcontext = MemoryContextSwitchTo(GetMemoryChunkContext(file));
		file->enc_ciphertext_buf = palloc(max_ciphertext);
		file->enc_plaintext_buf = palloc(BLCKSZ);
						  ...
						  bool missing_ok)
{
	...
	if ((Size) ciphertext_len != (Size) plaintext_len + FileEncryptionOverheadSize())
		ereport(ERROR,
				(errcode(ERRCODE_DATA_CORRUPTED),
				 errmsg("encrypted BufFile block header in \"%s\" is inconsistent with module overhead",
						FilePathName(thisfile))));

	ct_read = FileRead(thisfile, file->enc_ciphertext_buf,
	...
	FileEncryptionDecrypt(FilePathName(thisfile), phys_offset,
						  file->enc_ciphertext_buf, ciphertext_len,
						  file->enc_plaintext_buf);
						   ...
						   const char *data, uint32 plaintext_len)
{
	...
	ciphertext_len = plaintext_len + (uint32) FileEncryptionOverheadSize();
	FileEncryptionEncrypt(FilePathName(thisfile), phys_offset,
						  data, plaintext_len,
						  file->enc_ciphertext_buf);
		...
		ereport(ERROR,
				(errcode_for_file_access(),
				 errmsg("could not write to file \"%s\": %m",
						FilePathName(thisfile))));
	if ((uint32) FileWrite(thisfile, file->enc_ciphertext_buf,
						   ciphertext_len,
...
BufFileLoadEncryptedBlock(BufFile *file, bool for_write)
{
	...
	if (plaintext_len > 0)
	{
		memcpy(file->buffer.data, file->enc_plaintext_buf, plaintext_len);
...
BufFileClose(BufFile *file)
{
	...
	if (file->enc_ciphertext_buf != NULL)
	{
		pfree(file->enc_ciphertext_buf);
		pfree(file->enc_plaintext_buf);
	}
	pfree(file);
}
