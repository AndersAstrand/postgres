...
/*
 * Name of the module this process is bound to.  Same string
 * process_file_encryption_library() recorded; NULL/empty when no module is
 * configured.
 */
const char *
FileEncryptionLibraryName(void)
{
	return active_file_encryption_library;
}
...
process_file_encryption_library(const char *libname)
{
	...
	/*
	 * The module's only configuration channel is the opaque config string
	 * passed to _PG_file_encryption_module_init; modules must not define
	 * GUCs from their entry point.  We deliberately do not toggle
	 * process_shared_preload_libraries_in_progress here.
	 */
...
load_and_validate_module(const char *libname)
{
	...
	/*
	 * Fall back to the PGFILEENCRYPTIONCONFIG environment variable when the
	 * GUC is unset.  initdb forwards --file-encryption-config to its child
	 * backends through this env var rather than postgres's command line, so
	 * an operator-supplied configuration value isn't trivially observable
	 * via `ps` during initdb.
	 */
	module_config = file_encryption_config;
	if (module_config == NULL || module_config[0] == '\0')
		module_config = getenv("PGFILEENCRYPTIONCONFIG");
	 ...
	 */
	oldcontext = MemoryContextSwitchTo(TopMemoryContext);
	init_ok = (*init) (module_config, &callbacks, &init_errmsg);
	MemoryContextSwitchTo(oldcontext);
...
ensure_per_process_init(void)
{
	...
	/*
	 * Drop any pointer inherited from an ancestor process: the parent's
	 * per-process struct may have been freed by its shutdown callback
	 * before we forked, so this process must build its own.
	 */
