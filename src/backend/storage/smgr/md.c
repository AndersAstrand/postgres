...
md_readv_complete(PgAioHandle *ioh, PgAioResult prior_result, uint8 cb_data)
{
	...
	if (md_fork_is_encrypted(td->smgr.forkNum) && result.result > 0)
	{
		 ...
		 * this AIO already created (mdstartreadv / smgr_aio_reopen eagerly
		 * opened the encryption object using the same backend key), so this
		 * allocates nothing.  We must use the owning backend's procno for
		 * temp relations so the hash key matches; otherwise the lookup would
		 * mint a fresh SMgrRelation whose smgr_rlocator.backend is
		 * INVALID_PROC_NUMBER, which then drives relpath to construct a
		 * permanent-style "_key" path that does not exist on disk.
		 */
		procno = td->smgr.is_temp ? pgaio_io_get_owner(ioh) : INVALID_PROC_NUMBER;
		reln = smgropen(td->smgr.rlocator, procno);
