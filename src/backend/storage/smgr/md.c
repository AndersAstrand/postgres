...
md_init_enc_workspace(void)
{
	...
	if (FileEncryptionEnabled() && md_enc_workspace == NULL)
...
mdunlinkfork(RelFileLocatorBackend rlocator, ForkNumber forknum, bool isRedo)
{
	...
	/*
	 * KEY_FORKNUM: schedule a delayed unlink without truncating the file.
	 * The wrapped DEK has to stay readable until the relation's drop
	 * COMMIT is itself replayed: WAL records that touch the dropped
	 * relation are replayed first and any buffer flush during that
	 * replay needs the KEY fork to encrypt its bytes.  Other forks
	 * (FSM, VM) get re-created on-demand by their own WAL records, but
	 * nothing re-creates the KEY fork after RelationCreateStorage.  We
	 * don't truncate the file (unlike MAIN, where the do_truncate
	 * reclaims disk space proactively): KEY fork is a single BLCKSZ
	 * block, the space savings are negligible, and a zero-byte file
	 * would short-read when the next encrypted page tries to unwrap
	 * the DEK during recovery.
	 */
...
mdunlinkfiletag(const FileTag *ftag, char *path)
{
	...
	/*
	 * Compute the path.  Honor the forknum from the tag rather than
	 * hardcoding MAIN_FORKNUM: a KEY fork can also reach this path via
	 * mdunlinkfork's deferred-unlink branch, and unlinking MAIN instead
	 * would silently leak the KEY fork file.
	 */
...
md_readv_complete(PgAioHandle *ioh, PgAioResult prior_result, uint8 cb_data)
{
	...
	if (md_fork_is_encrypted(td->smgr.forkNum) && result.result > 0)
	{
		 ...
		 * state.  smgropen_existing is a pure hash lookup, never allocates,
		 * and returns NULL if the issuer didn't pre-open the entry --
		 * mandatory inside this completion-callback critical section.
		 * mdstartreadv (in the issuing backend) and smgr_aio_reopen (in IO
		 * workers) both populate the entry before the IO is dispatched.
		 *
		 * We must use the owning backend's procno for temp relations so the
		 * hash key matches; otherwise the lookup would miss and return NULL.
		 */
		procno = td->smgr.is_temp ? pgaio_io_get_owner(ioh) : INVALID_PROC_NUMBER;
		reln = smgropen_existing(td->smgr.rlocator, procno);
		if (reln == NULL)
			elog(PANIC, "md_readv_complete: SMgrRelation for %u/%u/%u missing at IO completion",
				 td->smgr.rlocator.spcOid,
