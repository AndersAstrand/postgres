...
ReorderBufferSerializeTXN(ReorderBuffer *rb, ReorderBufferTXN *txn)
{
	...
	dlist_foreach_modify(change_i, &txn->changes)
	{
			...
			!XLByteInSeg(change->lsn, curOpenSegNo, wal_segment_size))
		{
			 ...
			 *
			 * Safe because spill files are owned by a single decoding
			 * worker (the slot's): nothing else writes to this fd, so the
			 * locally tracked write_offset stays in lockstep with the
			 * kernel's append position.  A future caller introducing
			 * concurrent writers would have to abandon the offset
			 * tracking here.
			 */
			if (encrypted)
			{
							 ...
							 ReorderBufferChange *change)
{
	...
	if (FileEncryptionEnabled())
	{
		...
		ciphertext_size = ondisk->size + FileEncryptionOverheadSize();

		encrypted_record.plaintext_size = ondisk->size;
		encrypted_record.ciphertext_size = ciphertext_size;

		resetStringInfo(ciphertext);
		enlargeStringInfo(ciphertext, (int) ciphertext_size);
		FileEncryptionEncrypt(path, *write_offset,
							  rb->outbuf, ondisk->size, ciphertext->data);
		ciphertext->len = (int) ciphertext_size;
							...
							TXNEntryFile *file, XLogSegNo *segno)
{
	...
	while (restored < max_changes_in_memory && *segno <= last_segno)
	{
		...
		if (encrypted)
		{
			...
			if (encrypted_record.ciphertext_size !=
				encrypted_record.plaintext_size + FileEncryptionOverheadSize())
				ereport(ERROR,
						(errcode(ERRCODE_DATA_CORRUPTED),
						 errmsg("reorderbuffer spill record is inconsistent with module overhead")));
			if (encrypted_record.plaintext_size < sizeof(ReorderBufferDiskChange))
				ereport(ERROR,
						(errcode(ERRCODE_DATA_CORRUPTED),
						 errmsg("reorderbuffer spill record is truncated")));
			...
			resetStringInfo(&plaintext);
			enlargeStringInfo(&plaintext, (int) encrypted_record.plaintext_size);
			FileEncryptionDecrypt(path, record_offset,
								  ciphertext.data, ciphertext.len,
								  plaintext.data);
			plaintext.len = (int) encrypted_record.plaintext_size;

			ondisk = (ReorderBufferDiskChange *) plaintext.data;
			if (ondisk->size != plaintext.len)
				ereport(ERROR,
						(errcode(ERRCODE_DATA_CORRUPTED),
						 errmsg("reorderbuffer spill record header is inconsistent with its size")));
