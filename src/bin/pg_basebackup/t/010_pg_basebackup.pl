...
{
    plan skip_all => "pg_basebackup without -E from server with encrypted WAL produces broken backups";
}

if ($ENV{TDE_MODE_SMGR} and not $ENV{TDE_MODE_NOSKIP})
{
	plan skip_all => 'uses corrupt_page_checksum to directly hack relation files';
}
