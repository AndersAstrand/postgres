...
#ifdef PERCONA_EXT
/*
 * Identify the pg_tde directory.
 *
 * First try provided KRPATH
 *
 * Second try waldir/../pg_tde
 *
 * NULL if no viable pg_tde directory found
 */
static char *
identify_pg_tde_directory(char *kringdir, char *waldir)
{
	char		fpath[MAXPGPATH];

	if (kringdir != NULL)
	{
		if (search_directory(kringdir, "wal_keys"))
			return kringdir;
		else
			pg_fatal("wal_keys not found in provided keyring path \"%s\"", kringdir);
	}

	snprintf(fpath, MAXPGPATH, "%s/../pg_tde", waldir);

	if (search_directory(fpath, "wal_keys"))
			return pg_strdup(fpath);
...
#ifdef PERCONA_EXT
	printf(_("  -k, --keyring-path=KRPATH directory in which to find keyring config files for WAL\n"
			 "                            such files are wal_keys and %d_providers\n"
			 "                            (default: PATH/../pg_tde)\n"), GLOBAL_DATA_TDE_OID);
#endif
...
#ifdef PERCONA_EXT
		kringdir = identify_pg_tde_directory(kringdir, waldir);
		if (kringdir != NULL)
		{
