 ...
 * will live on disk; modules bind it into their per-call key/IV/MAC
 * derivation however they see fit (as AEAD AAD, HMAC input, an XTS-style
 * tweak, ...) so substituting one record for another at decrypt time
 * fails.  Each call is otherwise independent -- modules typically generate
 * a fresh data-encryption key per call and wrap it under the module's
 * configured KEK in the overhead.
 *
 ...
 *
 * object_open_cb returns a non-NULL pointer to module-owned state on
 * success and NULL on failure (with *errmsg populated).  The lifetime of
 * the returned pointer must outlive any encrypt/decrypt call referring to
 * the same relation; the host arranges for the allocator's lifetime to
 * match.
 */
 ...
 */
#define PG_FILE_ENCRYPTION_MAGIC 0x46454D38		/* "FEM8" */
