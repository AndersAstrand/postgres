...
/*
 * Name of the module that this process is bound to.  In bootstrap mode this
 * is whatever -L was passed; at runtime it tracks pg_control.  Returns NULL
 * (or "") when no module is configured.  Distinct from
 * GetFileEncryptionLibrary() in xlog.h, which always reads pg_control --
 * useful for code paths (like InitControlFile) that *populate* that field.
 */
extern const char *FileEncryptionLibraryName(void);

/* Per-call ciphertext overhead size declared by the loaded module. */
extern Size FileEncryptionOverheadSize(void);

/*
 * Encrypt data_len plaintext bytes into the caller-allocated dst (which
 * must hold data_len + FileEncryptionOverheadSize() bytes).  Decrypt is
 * symmetric: data_len input bytes (including trailing overhead) produce
 * exactly data_len - FileEncryptionOverheadSize() plaintext bytes.
 */
