...
sub	_tde_init_principal_key
{
	...
	my $tde_template_dir = $ENV{TDE_TEMPLATE_DIR} if defined($ENV{TDE_TEMPLATE_DIR});

	unless (defined($tde_template_dir))
	{
		$tde_template_dir = $PostgreSQL::Test::Utils::tmp_check . '/pg_tde_template';
		...
		unless (-e $tde_template_dir)
		{
			...
			_tde_init_sql_command($temp_dir, 'postgres', qq(
