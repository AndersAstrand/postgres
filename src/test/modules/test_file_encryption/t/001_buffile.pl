...
# self-verifying payload column round-trips.  Also verify the per-file
# init_file_cb / encrypt_cb / decrypt_cb callbacks fire by inspecting
# the log_summary line emitted on backend exit.
...
# Disconnect so the backend's before_shmem_exit callback flushes
# log_summary, then poll the server log for proof that init_file and
