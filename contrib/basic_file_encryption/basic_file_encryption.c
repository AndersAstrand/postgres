 ...
 *
 * This module is a demonstration and test target, not a production tool.
 * It accepts a 256-bit AES key-encryption key (KEK) directly as 64 hex
 * characters in the config string, which keeps the test setup trivial but
 * is not how a production-grade module should source its keys.  Real
 * modules would treat the config string as an opaque reference (e.g. a
 * KMS URI, a vault path, a file name) and fetch the actual key from an
 * external key store at startup_cb time.
 *
...
static bool bfe_startup(FileEncryptionModuleState *state, char **errmsg);
 ...
 * without going through that entry point, fail fast with an errmsg.
 */
static bool
bfe_startup(FileEncryptionModuleState *state, char **errmsg)
{
...
/*
 * The AAD is basename(path) || file_offset(be64).  Using only the basename
 * keeps CREATE DATABASE FILE_COPY and ALTER DATABASE SET TABLESPACE
 * round-trips working: both clone temp files into directories whose leading
 * components differ even though the basename is preserved.
 *
 * Tradeoff: two record-stream files that share a basename in different
 * directories produce AAD-equivalent ciphertexts.  An attacker with disk
 * write access to both could shuffle records between them undetected at
 * decrypt time.  Within a single PGDATA the temp file naming (e.g. the
 * BufFile per-PID counters in fd.c, the per-XID spill names in
 * reorderbuffer.c) makes that hard to engineer in practice, but operators
 * who need the stronger guarantee should pick a module that binds the full
 * path.
 */
static void
			...
			char *dst, char **errmsg)
{
	...
	p += BFE_IV_LEN;
	memcpy(p, data_tag, BFE_TAG_LEN);
	p += BFE_TAG_LEN;
	memcpy(p, wrap_iv, BFE_IV_LEN);
	p += BFE_IV_LEN;

	ok = bfe_aes_gcm_encrypt(priv->kek, wrap_iv,
							 bfe_aad_update_record, &aad,
							 data_key, BFE_KEY_LEN,
							 (unsigned char *) p, wrap_tag,
							 errmsg);
	explicit_bzero(data_key, sizeof(data_key));
	if (!ok)
		return false;
	...
	memcpy(p, wrap_tag, BFE_TAG_LEN);
	p += BFE_TAG_LEN;
	memcpy(p, &format, sizeof(format));
	p += sizeof(format);
	memset(p, 0, BFE_PAD_SIZE);
						...
						Size *wrapped_len, char **errmsg)
{
	...
	if (dst_max < BFE_OBJ_WRAP_SIZE)
	{
		*errmsg = psprintf("basic_file_encryption: wrapped-key buffer is %zu bytes, need %zu",
						   dst_max, (Size) BFE_OBJ_WRAP_SIZE);
		return false;
	}
