 ...
 *
 *   * Page encrypt/decrypt binds blocknum(be32) only.  The per-relation DEK
 *     already pins which relation we're decrypting; the AAD prevents
 *     intra-relation block substitution at a different blocknum.  Fork is
 *     NOT bound: reinit raw-copies INIT fork bytes into the MAIN fork on
 *     unlogged-relation reset, so MAIN block N must decrypt ciphertext
 *     that was originally produced for fork=INIT_FORKNUM.
 *
					...
					ForkNumber fork, BlockNumber blocknum)
{
	...
	/*
	 * Bind blocknum only.  Fork is deliberately NOT included in the AAD:
	 * reinit of an unlogged relation raw-copies INIT fork bytes into the
	 * MAIN fork on crash recovery, so MAIN block N must decrypt ciphertext
	 * that was produced when fork=INIT_FORKNUM.  Block-position binding is
	 * still enforced via blocknum.
	 */
	(void) fork;
	for (int i = 0; i < 4; i++)
		buf[i] = (unsigned char) (blocknum >> ((3 - i) * 8));
	if (update(ctx, NULL, &outlen, buf, 4) != 1)
