 ...
 *   * Page:            blocknum(be32)  (fork deliberately NOT bound -- see
 *                     comment in pfem_encrypt_page)
 */
static void
				  ...
				  const char *src, char *dst)
{
	...
	/*
	 * AAD = blocknum(be32) only.  Fork is NOT bound: reinit raw-copies the
	 * INIT fork bytes into the MAIN fork on unlogged-relation reset, so
	 * MAIN block N must decrypt ciphertext produced for fork=INIT_FORKNUM.
	 */
	(void) fork;
	pfem_store32_be(aad, blocknum);
				  ...
				  const char *src, char *dst)
{
	...
	/* AAD = blocknum only -- see comment in pfem_encrypt_page. */
	(void) fork;
	pfem_store32_be(aad, blocknum);
