				  ...
				  const char *src, char *dst)
{
	...
	/*
	 * AAD = fork(be32) || blocknum(be32).  Including the fork distinguishes
	 * MAIN block N from INIT block N so neither can be substituted for the
	 * other on disk; reinit re-encrypts INIT bytes under MAIN's AAD when
	 * copying INIT into MAIN on unlogged-relation reset.
	 */
				  ...
				  const char *src, char *dst)
{
	...
	/* AAD = fork(be32) || blocknum(be32) -- see comment in pfem_encrypt_page. */
	pfem_store32_be(aad, (uint32) fork);
	pfem_store32_be(aad + 4, blocknum);
