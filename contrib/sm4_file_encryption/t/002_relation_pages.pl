...
$node->init(extra => ['--file-encryption-page-reserved-size=56',
					  '-c', 'file_encryption_library=sm4_file_encryption',
					  '-c', "sm4_file_encryption.key=$key",
					  '-c', 'io_method=sync']);
$node->append_conf('postgresql.conf', qq[
file_encryption_library = 'sm4_file_encryption'
sm4_file_encryption.key = '$key'
io_method = 'sync'
]);
$node->start;
