...
#define SM4_KEY_GUC			"sm4_file_encryption.key"
#define SM4_PROVIDER_GUC	"sm4_file_encryption.provider"
#define SM4_CIPHER_GUC		"sm4_file_encryption.cipher"
#define SM4_DIGEST_GUC		"sm4_file_encryption.digest"
...
static bool
sm4_check_key(char **newval, void **extra, GucSource source)
{
	...
	if (strlen(str) != SM4_KEY_LEN * 2)
	{
		GUC_check_errdetail("%s must be %d hex characters (got %zu).",
							SM4_KEY_GUC, SM4_KEY_LEN * 2, strlen(str));
		return false;
	}
...
_PG_init(void)
{
	DefineCustomStringVariable(SM4_KEY_GUC,
							   ...
							   PGC_POSTMASTER,
							   GUC_SUPERUSER_ONLY,
							   sm4_check_key, NULL, NULL);

	DefineCustomStringVariable(SM4_PROVIDER_GUC,
							   "OpenSSL provider to load before resolving the cipher and digest.",
							   NULL,
							   &sm4_provider,
							   "",
							   PGC_POSTMASTER,
	...
	DefineCustomStringVariable(SM4_CIPHER_GUC,
	...
	DefineCustomStringVariable(SM4_DIGEST_GUC,
	...
	MarkGUCPrefixReserved("sm4_file_encryption");
}

const FileEncryptionCallbacks *
_PG_file_encryption_module_init(void)
{
	return &sm4_file_encryption_callbacks;
}
...
sm4_startup(FileEncryptionModuleState *state)
{
	...
	decoded = hex_decode(sm4_key, strlen(sm4_key), (char *) priv->kek);
	...
	if (priv == NULL)
		ereport(ERROR,
				(errcode(ERRCODE_CONFIG_FILE_ERROR),
				 errmsg("%s is not set", SM4_KEY_GUC)));
	return priv;
}
	...
	ereport(ERROR,
			(errcode(ERRCODE_CONFIG_FILE_ERROR),
			 ...
			 errhint("Install or activate an OpenSSL provider that implements it. "
					 ...
					 SM4_PROVIDER_GUC, SM4_CIPHER_GUC, SM4_DIGEST_GUC)));
}
...
sm4_load_crypto(SM4FileEncryptionState *priv)
{
	...
	if (sm4_cipher == NULL || sm4_cipher[0] == '\0')
		ereport(ERROR,
				(errcode(ERRCODE_CONFIG_FILE_ERROR),
				 errmsg("%s must not be empty", SM4_CIPHER_GUC)));
	if (sm4_digest == NULL || sm4_digest[0] == '\0')
		ereport(ERROR,
				(errcode(ERRCODE_CONFIG_FILE_ERROR),
				 errmsg("%s must not be empty", SM4_DIGEST_GUC)));
