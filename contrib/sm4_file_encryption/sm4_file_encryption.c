 ...
 * It accepts a hex-encoded SM4 key-encryption key (KEK) directly via the
 * config blob, which keeps the test setup trivial but is not how a
 * production-grade module should source its keys.  Real modules would
 * treat the config string as an opaque reference (e.g. a KMS URI, a
 * vault path, a file name) and fetch the actual key from an external
 * key store at startup_cb time.
 *
...
static bool sm4_startup(FileEncryptionModuleState *state, char **errmsg);
...
/*
 * Per-process startup.  Reports load-time failures (provider not loadable,
 * cipher/digest unavailable, HMAC tag length mismatch) up to the host via
 * *errmsg.
 */
static bool
sm4_startup(FileEncryptionModuleState *state, char **errmsg)
{
	...
	if (!sm4_load_crypto(priv, errmsg))
	{
			...
			char *dst, char **errmsg)
{
	...
	if (!sm4_cipher_crypt_raw(priv, true, data_enc_key, data_iv,
							  (const unsigned char *) data, body_len,
							  (unsigned char *) dst, errmsg))
		goto out;
	if (!sm4_hmac(priv, data_mac_key, SM4_KEY_LEN, path, file_offset,
				  (const unsigned char *) dst, body_len, data_tag, errmsg))
		goto out;

	p = dst + body_len;
	memcpy(p, data_iv, SM4_IV_SLOT_LEN);
	p += SM4_IV_SLOT_LEN;
	memcpy(p, data_tag, SM4_TAG_LEN);
	p += SM4_TAG_LEN;
	memcpy(p, wrap_iv, SM4_IV_SLOT_LEN);
	p += SM4_IV_SLOT_LEN;

	if (!sm4_cipher_crypt_raw(priv, true, priv->kek, wrap_iv,
							  key_material, SM4_KEY_MATERIAL_LEN,
							  (unsigned char *) p, errmsg))
		goto out;
	if (!sm4_hmac(priv, priv->kek, SM4_KEY_LEN, path, file_offset,
				  (const unsigned char *) p, SM4_KEY_MATERIAL_LEN,
				  wrap_tag, errmsg))
		goto out;
	p += SM4_KEY_MATERIAL_LEN;

	memcpy(p, wrap_tag, SM4_TAG_LEN);
	p += SM4_TAG_LEN;
	memcpy(p, &format, sizeof(format));
	p += sizeof(format);
	if (SM4_PAD_SIZE > 0)
		memset(p, 0, SM4_PAD_SIZE);
	ok = true;
			...
			char *dst, char **errmsg)
{
	...
	if (!sm4_cipher_crypt_raw(priv, false, key_material, data_iv,
							  (const unsigned char *) data, body_len,
							  (unsigned char *) dst, errmsg))
		goto out;
	ok = true;
						...
						Size *wrapped_len, char **errmsg)
{
	...
	if (dst_max < SM4_OBJ_WRAP_SIZE)
	{
		*errmsg = psprintf("sm4_file_encryption: wrapped-key buffer is %zu bytes, need %zu",
						   dst_max, (Size) SM4_OBJ_WRAP_SIZE);
		return false;
	}
