...
pg_tde_create_smgr_key_temp(const RelFileLocator *newrlocator)
{
	...
	if (TempRelKeys == NULL)
	{
		...
		TempRelKeys = hash_create("pg_tde temporary relation keys", 16,
 ...
 * (start) so there should be no XLog records, aquired locks. The key is always
 * created with start_lsn = InvalidXLogRecPtr. Which will be updated with the
 * actual lsn by the first WAL write.
 */
void
