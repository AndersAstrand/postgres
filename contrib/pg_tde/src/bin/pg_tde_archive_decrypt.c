...
usage(const char *progname)
{
	...
	printf(_("  ARCHIVE-COMMAND archive command to wrap, %%p will be replaced with the\n"
			 "                  absolute path of the decrypted WAL segment, %%f will be\n"
			 "                  replaced with only the filename.\n"));
	printf(_("\n"));
	printf(_("Note that any %% parameters in ARCHIVE-COMMAND will have to be escaped if used\n"
		     "as archive_command in postgresql.conf.\n"
			 "e.g.\n"
			 "  archive_command='%s %%p \"cp %%%%p /mnt/server/archivedir/%%%%f\"'\n"
			 "or\n"
			 "  archive_command='%s %%p \"pgbackrest --stanza=your_stanza archive-push %%%%p\"'\n"), progname, progname);
